...
class AnthropicClient(BaseLLMClient):
    ...
    def _normalize_claude_response(
        ...
        if tool_calls:
            ...
            logging.debug(f"Normalized {len(tool_calls)} tool calls from Claude response")
            ...
            with httpx.Client(timeout=self.timeout) as client:
                ...
                if response.status_code >= 400:
                    try:
                        ...
                        print("\n" + "="*80, file=sys.stderr)
                        print(f"Failed to parse Claude error JSON: {e}", file=sys.stderr)
                ...
                if logging.getLogger().isEnabledFor(logging.DEBUG):
                    try:
                        logging.debug(f"Claude raw response: {json.dumps(data, indent=2)}")
                    except Exception as e:
                        logging.debug(f"Couldn't serialize Claude response for logging: {e}")
                
                # Normalize Claude response to OpenAI format
                openai_compatible_response = self._normalize_claude_response(data)
                
                logging.debug(f"Converted Claude response: {openai_compatible_response}")
                return openai_compatible_response
                
        except httpx.RequestError as e:
            logging.error(f"Error communicating with Claude API: {e}")
            return {"role": "assistant", "content": f"Error: Could not reach Claude API: {e}"}
