...
def setup_logging(log_file=None, level=DEFAULT_LOG_LEVEL, run_id=None, quiet_console=True, output_only=False):
    """
...
def log_http_response(logger, url, status_code, headers, body=None, response_time=None, extra=None):
...
def log_llm_request(logger, provider, model, messages, tools=None, temperature=None, extra=None):
...
def log_tool_response(logger, tool_name, response, success=True, time_ms=None, taskName=None, extra=None):
    """Log a tool response."""
    payload = {
        "tool": tool_name,
        "success": success,
...
def sanitize_headers(headers):
    ...
    sensitive_keys = ['authorization', 'x-api-key', 'api-key', 'x-openai-api-key', 'anthropic-api-key']
...
class LoggingTimer:
    ...
    def __init__(self, logger, operation_name, category=CATEGORY_PERFORMANCE, extra=None):
        self.logger = logger
